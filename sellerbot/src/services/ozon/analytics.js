// src/services/ozon/analytics.js
...
async function fetchStocksPositiveBySku({ client_id, api_key }) {
  ...
  while (true) {
    const data = await ozonApiRequest({ client_id, api_key, endpoint, body: { limit, offset } });
...
/** /v1/analytics/data — с фильтром по SKU. Возвращает [revenue, ordered_units] */
async function getOzonReportFiltered({ client_id, api_key, date, metrics, trackedSkus }) {
  ...
  if (!list) {
    const data = await ozonApiRequest({
      client_id, api_key, endpoint: '/v1/analytics/data',
      body: { date_from: date, date_to: date, metrics, dimension: ['day'], filters: [], limit: 1, offset: 0 },
    });
    if (!data?.result?.data?.length) return [0, 0];
    return data.result.data[0].metrics;
  }

  let revenue = 0, ordered = 0;
  for (const sku of list) {
    try {
      const resp = await ozonApiRequest({
        client_id, api_key, endpoint: '/v1/analytics/data',
        body: {
          date_from: date, date_to: date, metrics, dimension: ['day'],
          filters: [{ key: 'sku', value: String(sku), operator: '=' }], limit: 1, offset: 0,
        },
      });
      const m = resp?.result?.data?.[0]?.metrics;
      if (Array.isArray(m) && m.length >= 2) { revenue += Number(m[0] || 0); ordered += Number(m[1] || 0); }
    } catch (e) {
...
/** СВД (дни). POST /v1/analytics/average-delivery-time/summary */
async function getAverageDeliveryTimeDays({ client_id, api_key }) {
  const data = await ozonApiRequest({
    client_id, api_key, endpoint: '/v1/analytics/average-delivery-time/summary', body: {},
  });
  const v = data?.result?.average_delivery_time ?? data?.average_delivery_time ?? null;
  const n = Number(v);
  return Number.isFinite(n) ? n : null;
}

/** Суммарные остатки по списку SKU. POST /v1/analytics/stocks { skus: [...] } */
async function getStocksSumBySkus({ client_id, api_key, skus }) {
  const list = (Array.isArray(skus) ? skus : []).map(String).filter(Boolean);
  if (!list.length) return new Map();

  const data = await ozonApiRequest({ client_id, api_key, endpoint: '/v1/analytics/stocks', body: { skus: list } });
...
/** Заказано/выручка по SKU за дату. Возврат: Map<sku, { ordered, revenue }> */
async function getOrderedBySkuMap({ client_id, api_key, date, trackedSkus = null }) {
  const resp = await ozonApiRequest({
    client_id, api_key, endpoint: '/v1/analytics/data',
    body: { date_from: date, date_to: date, metrics: ['revenue','ordered_units'], dimension: ['sku'], limit: 1000, offset: 0 },
  });
  ...
  for (const row of rows) {
    const skuStr = row?.dimensions?.[0]?.value ?? row?.dimensions?.[0]?.id ?? row?.dimensions?.[0];
    ...
    map.set(sku, { ordered: Number(m?.[1] || 0), revenue: Number(m?.[0] || 0) });
  }
  return map;
}
