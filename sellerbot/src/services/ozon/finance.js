...
/** Доставка покупателю — агрегаты по выкупам. */
async function getDeliveryBuyoutStats({
  ...
  while (true) {
    const data = await ozonApiRequest({
      ...
      body: {
        filter: { date: { from: date_from, to: date_to }, operation_type: [], posting_number: '', transaction_type: 'all' },
        page, page_size,
      },
    });
    ...
    for (const op of ops) {
      ...
      if (acc > 0) { totalCount += 1; totalAmount += acc; }
...
/** Разбивка продаж по SKU (для вторых сообщений/детализации, если пригодится) */
...
async function getBuyoutAndProfit({ client_id, api_key, date_from, date_to, buyoutCost, buyoutAmount }) {
  const data = await ozonApiRequest({
    client_id, api_key, endpoint: '/v3/finance/transaction/totals',
    body: { date: { from: date_from, to: date_to }, posting_number: '', transaction_type: 'all' },
  });
