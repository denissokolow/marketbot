...
// Метрики и индикации идентичны /last30: Заказано / Выкуплено / Доставляется / Возвраты / Отмены / Брак / % выкупа / Д.Р.Р / CTR / Расходы / Прибыль / Прибыль на шт. / ROI / ABC.
...
// ВАЖНО: внутри <code> — только «чистый» текст
...
// ---------- DEBUG/ретраи (те же имена ENV, что в /last30) ----------
...
// ---------- сплит-параметры (те же, что для /last30) ----------
...
async function buildLastMDataAndBlocks(user, { trackedSkus = [], db = null, chatId = null } = {}) {
  ...
  if (DEBUG_LAST30) console.log('[LASTM] range', { fromYmd, toYmd });
  ...
  // Веса для кампаний «все товары»: пропорционально брутто-выручке SKU
  ...
  // 5) Рекламные метрики per-SKU (views/clicks/spent)
...
/** Единым сообщением (обратная совместимость) */
async function makeLastMPerSkuText(user, opts = {}) {
  const { headerBlock, skuBlocks, totalBlock } = await buildLastMDataAndBlocks(user, opts);
  return [headerBlock, ...skuBlocks, totalBlock].join('\n');
}

/** Возвращает МАССИВ сообщений (чанков) для безопасной отправки в Telegram.
 * Опции:
 ...
 */
async function makeLastMPerSkuMessages(user, opts = {}) {
  const { headerBlock, skuBlocks, totalBlock } = await buildLastMDataAndBlocks(user, opts);
  ...
  // total — к последнему чанку, либо отдельным
