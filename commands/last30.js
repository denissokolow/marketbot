// commands/last30.js
const { makeLast30PerSkuText } = require('../utils/reportMtdSku');
...
module.exports = (bot, db) => {
  const handler = async (ctx) => {
    try {
      ...
      if (!userRow) return ctx.reply('Пользователь не найден. Нужна регистрация (/start).');
      ...
      if (!trackedSkus.length) {
        return ctx.reply('Нет отслеживаемых товаров для отчёта за 30 дней.');
      }

      const user = {
        shop_name: userRow.shop_name || 'Неизвестно',
        client_id: userRow.client_id,
        seller_api: userRow.seller_api,
      };

      await ctx.replyWithChatAction('typing');

      const text = await makeLast30PerSkuText(user, {
        trackedSkus,
        db,
      ...
      console.error('[command /last30] error:', e?.response?.data || e);
      return ctx.reply('Не удалось сформировать отчёт за 30 дней.');
    }
  };

  bot.command('last30', handler);
  bot.command('mtd30', handler); // алиас
};
